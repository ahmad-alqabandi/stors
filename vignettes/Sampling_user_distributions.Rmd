...
To sample from user-defined distributions, you can create custom grids using the `build_proposal()` function and then use the `stors()` function to create a sampler based on the custom grid.
...
After building the desired grid with `build_proposal()`, you can explore its properties using the `print()` function and visualize its structure using the `plot()` function. This step is similar to what was done previously with the built-in grid and helps in understanding the grid's configuration and ensuring that it aligns with the requirements of the target distribution.
...
plot(
  bi_grid_trunc_grid,
  main = "Truncated Multi-Modal Grid (-1, 6)",
  xlab = "Value",
  col = "skyblue"
)
...
The grid stored in `bi_grid` will not be preserved across sessions. Therefore, if you are satisfied with the sampling results and wish to store the grid for future use, you should save the grid using the `save_proposal()` function. This function takes the optimized grid and a filename you choose to save the grid in the internal R data directory on your machine. This is recommended if you plan to use the grid frequently across sessions, to avoid re-optimizing the grid in future sessions.

```{r , echo = TRUE}
save_proposal(bi_grid, "bimodal_grid")
```

To load and use the grid in future sessions, you only need to use the `load_proposal()` function.

```{r , echo = TRUE}
bi_grid <- load_proposal("bimodal_grid")
```

In case you want to view a list of all previously stored grids, the `print_proposals()` function will provide you with all stored grids.
