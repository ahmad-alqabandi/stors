...
The \code{proposal_optimizer} functions not only optimize the grid but also return a detailed list describing the properties of the optimized grid. This list provides insights into the grid's structure and characteristics. You can use the base \code{print()} function to view these details or the \code{plot()} function to visualize the grid.
...
These functionalists of the proposal_optimizers and the subsequent visualization tools provide users with in-depth insights into the characteristics of the generated grid, enabling them to understand and fine-tune the sampling process.


If the user is not satisfied with the automatically optimized grid, the proposal_optimizers functions offers several arguments for customization. One such argument is `target_sample_size`, which defaults to 1000. If the user wishes to optimize the grid for significantly larger or smaller sample sizes, this argument can be altered.
...
delete_build_in_grid(sampling_function = "srnorm", proposal_type = "custom")
...
bi_grid <- build_proposal(lb = -Inf, rb = Inf, modes = modes_bi, f = f_bi, h = h_bi, h_prime = h_prime_bi)

plot(bi_grid)

```

After building the desired grid with `build_proposal()`, users can explore its properties using the `print()` function and visualize its structure using the `plot()` function. This step is similar to what was done previously with the built-in grid and helps in understanding the grid's configuration and ensuring that it aligns with the requirements of the target distribution.
...
bi_grid_trunc_grid <- build_proposal(
  lb = -1,
  rb = 6,
  modes = modes_bi,
  f = f_bi,
...
The grid stored in `bi_grid` will not be preserved across sessions. Therefore, if the user is satisfied with the sampling results and wishes to store the grid for future use, they should save the grid using the `save_proposal()` function. This function takes the optimized grid and a filename chosen by the user to save the grid in the internal R data directory on their machine. This is recommended if the user plans to use the grid frequently across sessions, to avoid re-optimizing the grid in future sessions.

```{r , echo = TRUE}

save_proposal(bi_grid, "bimodal_grid")

```

```{r , include = FALSE}
for (i in 1:3) {
  save_proposal(bi_grid, paste0("bimodal_grid_", i))
}

```

To load and use the grid in future sessions, the user only needs to use the `load_proposal()` function.

```{r , echo = TRUE}

bi_grid <- load_proposal("bimodal_grid")

```

In case the user wants to view a list of all previously stored grids, the `print_proposals()` function will provide the user with all stored grids.
