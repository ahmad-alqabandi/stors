...
With the optimized grid ready, sampling from the standard normal distribution can be easily achieved using the `srnorm` function. For more general normal distributions, where the mean and standard deviation are not zero and one respectively, the `srsnorm` function can be used.
...
general_normal_samples = srsnorm(10, mu = 4, sd = 2)
...
truncated_normal_sampler = truncsrnorm(xl = lower_bound, xr = upper_bound)
