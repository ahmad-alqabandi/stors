...
To sample from any built-in distribution, the user only needs to run the `grid_optimizer()` function once when the package is loaded for the first time. This creates an optimized grid and store it in the internal R data directory, so it can be loaded and used for sampling when needed. Also you can visiulise the grid data using `plot()` function.
